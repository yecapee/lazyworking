...
//----ajax----------
	...
	if( !$.tmpPaser ) {
		$.tmpPaser = function (text){
			return	function(repObj){
						var rsText = text;
						
							for(var key in repObj){
								var re = new RegExp("{"+key+"}","g");
									rsText = rsText.replace(re,repObj[key]);
							}
...
function getType(bundleName){
	return bundleName.match(/(.*)_/)[1].replace('_','').toUpperCase();
}
